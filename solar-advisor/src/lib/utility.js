...
exports.getUtilityRates = getUtilityRates;
...
async function getUtilityRates(geoData, providedUtility) {
    ...
    if (providedUtility) {
        ...
        for (const state in utilityDatabase) {
            const utility = utilityDatabase[state].find(u => u.name.toLowerCase().includes(providedUtility.toLowerCase()));
            if (utility) {
                return {

interface InstallerSearchParams {
    lat: number;
    lng: number;
    maxDistance: number;
    minRating: number;
  ...
  export async function findLocalInstallers(params: InstallerSearchParams): Promise<Installer[]> {
    console.log(`Finding installers near ${params.lat}, ${params.lng}`);
    
    // Filter installers by rating and distance
    let filteredInstallers = mockInstallers.filter(installer => 
      installer.rating >= params.minRating &&
    ...
    if (params.preferredBrands && params.preferredBrands.length > 0) {
      filteredInstallers = filteredInstallers.filter(installer => 
        installer.specialties.some(specialty => 
          params.preferredBrands!.some(brand => 
            specialty.toLowerCase().includes(brand.toLowerCase())
          )
        ) ||
        installer.certifications.some(cert => 
          params.preferredBrands!.some(brand => 
            cert.toLowerCase().includes(brand.toLowerCase())
          )
        )
      );
    }
